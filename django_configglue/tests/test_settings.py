...
class SettingsCommandTestCase(ConfigGlueDjangoCommandTestCase):
    ...
    def test_locate_setting_no_configglue_parser(self):
        ...
        elif hasattr(settings, '_target'):
            wrapped = settings._target
        else:
            self.fail('Cannot find wrapped settings object')
        ...
        dirname = os.path.dirname(os.path.realpath(
            os.path.join(os.path.curdir, 'settings.py')))
        location = os.path.join(dirname, 'settings.pyc')
